<div class="bg-white rounded-lg shadow-lg p-6 space-y-6">
    <div class="flex items-start justify-between mb-8">
        ...
        <div class="flex space-x-4 items-center">
            ...
            <a href="{{ route('superadmin.crimes.prisoner-crimes.attach', [$prisoner->id]) }}"
            ...
            <a href="{{ route('admin.crimes.prisoner-crimes.attach', [$prisoner->id]) }}"
                class="px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-semibold rounded-md shadow-sm hover:from-blue-600 hover:to-blue-700 transition duration-200 ease-in-out inline-block">
                Add Crime
            </a>
            @elseif (Auth::user()->role === 'employee')
            <a href="{{ route('employee.crimes.prisoner-crimes.attach', [$prisoner->id]) }}"
                class="px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-semibold rounded-md shadow-sm hover:from-blue-600 hover:to-blue-700 transition duration-200 ease-in-out inline-block">
                Add Crime
            </a>
            @endif
        ...
        <table class="w-full table-auto border-collapse">
            <thead>
                <tr class="bg-gray-200 text-gray-700 font-semibold text-sm uppercase tracking-wide">
                    <th class="px-6 py-4 text-left">Crime</th>
                    <th class="px-6 py-4 text-left">Date Comitted</th>
                    <th class="px-6 py-4 text-left">Additional Notes</th>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-100 transition-colors duration-150">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-800">{{$crime->pivot->committed_date}}</td>
                    <td class="px-6 py-4 text-sm text-gray-800">{{$crime->pivot->additional_notes ?? 'None'}}</td>
                    <td class="px-10 py-4 flex space-x-2">
                        @if (Auth::user()->role === 'superadmin')
                        <a href="{{ route('superadmin.crimes.prisoner-crimes.update', [$crime->pivot->id]) }}"
                        ...
                        @elseif (Auth::user()->role === 'admin')
                        <a href="{{ route('admin.crimes.prisoner-crimes.update', [$crime->pivot->id]) }}"
                            class="px-3 py-1 bg-gradient-to-r from-green-500 to-green-600 text-white font-semibold rounded-md shadow-sm hover:from-green-600 hover:to-green-700 transition duration-200 ease-in-out inline-block">
                            Update
                        </a>
                        @elseif (Auth::user()->role === 'employee')
                        <a href="{{ route('employee.crimes.prisoner-crimes.update', [$crime->pivot->id]) }}"
                            class="px-3 py-1 bg-gradient-to-r from-green-500 to-green-600 text-white font-semibold rounded-md shadow-sm hover:from-green-600 hover:to-green-700 transition duration-200 ease-in-out inline-block">
                            Update
                        </a>
                        @endif
                        @if (Auth::user()->role === 'superadmin')
                        <button wire:click="detachCrime({{ $crime->pivot->id }})"
                            wire:confirm="Are you sure you want to remove this crime record?"
